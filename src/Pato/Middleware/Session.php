...
class Pato_Middleware_Session {
    ...
    function process_request(&$request) {
        ...
        if (isset($data[$user->session_key]) && isset($data[$user->session_key]['model'])) {
            ...
            $found_user = new $user_model($data[$user->session_key]['codigo']);
            if ($found_user->codigo == $data[$user->session_key]['codigo']) {
                // User found!
    ...
    function process_response($request, $response) {
        if ($request->session->touched) {
            ...
            if (!$request->user->isAnonymous ()) {
                $data[$request->user->session_key] = array ('codigo' => $request->user->codigo, 'model' => get_class ($request->user));
            } else {
    ...
    public static function processContext($signal, &$params) {
        $params['context'] = array_merge($params['context'],
                                         Pato_Middleware_Session_ContextPreProcessor($params['request']));
    }
}
 ...
 */
function Pato_Middleware_Session_ContextPreProcessor($request)
{
...
Gatuf_Signal::connect('Gatuf_Template_Context_Request::construct',
                     array('Pato_Middleware_Session', 'processContext'));
